"""Process-local, TTL-bound data-encryption-key (DEK) cache.

A user's DEK is unwrapped once at login and held in memory for the
lifetime of the access token (sliding expiry on every authenticated
request), so the key-encryption key is never re-derived per request.

The cache is per-process.  A multi-replica deployment must move it to a
shared store (Redis) or pin sessions to one replica.
"""
...
# Per-request DEK, set by the auth middleware after a cache lookup and
# cleared in the request's finally block.
...
def dek_scope(dek: bytes | None):
    ...
    Always restores the previous value, including when *dek* is
    ``None``, so the contextvar cannot leak across reused threads.
    """
...
class DekCache:
    ...
    def __init__(self, default_ttl: int = 900) -> None:
        """*default_ttl* is in seconds (matches a 15-minute access token)."""
        self._lock = threading.Lock()
        self._entries: dict[int, tuple[bytes, float]] = {}
        self._default_ttl = default_ttl

    def get(self, account_id: int) -> bytes | None:
        """Return the cached DEK when present and unexpired."""
        with self._lock:
    ...
    def evict(self, account_id: int) -> None:
        """Drop an entry immediately (logout, token-version bump)."""
        with self._lock:
...
def cache_set(account_id: int, dek: bytes) -> None:
    """Store *dek* in the process cache."""
    _dek_cache.set(account_id, dek)


def cache_touch(account_id: int) -> bool:
    """Refresh the TTL for *account_id*; False when not cached."""
